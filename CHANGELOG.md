...
- [#205](https://github.com/2listic/dealiiX-platform/pull/205) New pipeline editor: a higher-level visual DAG whose nodes are whole CORAL graphs or standalone executables, connected by ordering dependencies. A central "Pipeline editor" view (toggled from the top-right) lets you add stages (a CORAL graph from a file or the current canvas, or an executable), configure each stage's own resources (MPI nodes/tasks, time limit), and run the whole pipeline. On execution each stage is submitted as its own Slurm job chained after its predecessors, so stages run in the correct order, independent branches run in parallel, and the run continues even if the app is closed. Connections that would create a cycle are rejected. This first version runs in remote (Slurm) mode only and passes ordering between stages (no file hand-off yet).

### UI/UX

- [#222](https://github.com/2listic/dealiiX-platform/pull/222) The MPI toggle no longer appears for local runs. It had no effect there — the run was always serial — while still displaying a rank count, so a local run could look parallel when it was not. Local MPI support is tracked in [#221](https://github.com/2listic/dealiiX-platform/issues/221).
...
- [#223](https://github.com/2listic/dealiiX-platform/pull/223) Remote MPI jobs can now start their ranks through Slurm's own launcher (`srun`), which becomes the default for remote runs; `mpirun` remains available and stays the default for local runs. Whichever launcher is used, the job no longer restates the rank count it was allocated, so an MPI run can no longer silently fall back to a single rank, and it no longer passes an OpenMPI-only flag that would abort the job on clusters running a different MPI. Under `srun` the ranks also appear in the job accounting as a real step against the binary, where previously the whole run was accounted as a single task.
- [#223](https://github.com/2listic/dealiiX-platform/pull/223) Settings gains an MPI launcher section for the remote target: which launcher to use and any extra arguments to pass it, so a cluster that needs a specific PMI plugin can be given `--mpi=<plugin>` without a new app release. **Validate & Sync** also warns when the chosen launcher is not available on the target — reported as a warning in its own right rather than a plain success — instead of letting that surface as a job that fails after submission.
- [#223](https://github.com/2listic/dealiiX-platform/pull/223) Validations recorded by an earlier version are no longer carried over, so each location and mode asks for one **Validate & Sync** after upgrading.
- [#222](https://github.com/2listic/dealiiX-platform/pull/222) Executable runs can now use MPI, not just CORAL graph runs. When enabled, the job requests the nodes and tasks per node you configure and launches the binary through `mpirun`. Because the app supplies only the launcher and cannot tell whether a binary initialises MPI itself, the control is labelled "Binary is MPI-enabled" and defaults to off — ticking it for a non-MPI binary runs N independent copies over the same output directory.
- [#222](https://github.com/2listic/dealiiX-platform/pull/222) Fixed single executable runs ignoring the time limit: the field was only offered for CORAL runs, so every executable job was submitted with a one-hour limit regardless of what was configured. The field is now shown for both, and the value entered reaches the scheduler.
