...
- Fixed the Jobs table's Working Directory column being empty for local runs (both coral and executable) — it now shows the run's actual output directory, matching remote runs.
- The Jobs table now refreshes automatically when you switch execution location (local/remote) from the top-right selector. It no longer refreshes as a side effect of saving Settings, which could surface an unrelated connection error for a target you weren't actively using.
