...
- [#223](https://github.com/2listic/dealiiX-platform/pull/223) Remote MPI jobs now start their ranks through Slurm's own launcher (`srun`) instead of `mpirun`. The job no longer restates the rank count it was allocated, so an MPI run can no longer silently fall back to a single rank, and it no longer passes an OpenMPI-only flag that would abort the job on clusters running a different MPI. The ranks also appear in the job accounting as a real step against the binary, where previously the whole run was accounted as a single task.
