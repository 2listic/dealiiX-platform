...
- **Git hooks (Husky)**: On commit — `npm run lint` (failures abort), then Prettier auto-formats. On push — `npm run check` (svelte-check) and `npm run test`; either failure aborts the push.
