...
- `registryStore.svelte.ts` - Registry for available node types (both standard and network). Standard nodes are kept **per execution location** (`local`/`remote`), resolved by an internal `activeLocation` set via `setActiveLocation(location)` (no `settingsState` import — App syncs it via an effect). Exports `setRegistry()` (replace the active location's registry), `mergeRegistry()` (merge into it — used by manual "Import Nodes"), `getNodeData(type)`, `getAvailableNodes()`, `isNodeInRegistry(type)` for standard nodes; and `addNetworkNode(key, data)`, `removeNetworkNode(name)`, `getNetworkNodeDefinition(name)`, `getStoredNetworkNodes()`, `isNodeInNetworkNodes(name)` for subgraph (location-agnostic) nodes. Persisted in electron-store (`registered_nodes` as `{ local, remote }`, with best-effort migration from the old flat shape; `registered_network_nodes`) and loaded at startup.
...
- `parametersStore.svelte.ts` - Store for the executable/CORAL parameter tree, kept **per execution location** and persisted (`execution_parameters`), resolved by an internal `activeLocation` via `setActiveLocation(location)`. `parametersState.value` (get/set) and `parametersState.snapshot` (non-reactive copy) both resolve against the active location.
...
- `settingsStore.svelte.ts` - User settings persisted under a single `'settings'` key in electron-store (`AppSettings`: execution `location`/`backendKind`, per-target `local`/`remote` paths, and each target's per-backend-kind `probes` status). `settingsState` exposes getters (`execution`, `remote`, `local`, `isCoralMode`, `isExecutableMode`, `activeParametersFileName`, `activeProbe`, `getProbe(location, kind)`) and async setters: lightweight `setExecutionLocation(location)` / `setBackendKind(kind)` (mode switch, no probe), `saveExecutionPaths(execution)` (persist edited paths while preserving probe status), `recordProbe(location, kind, status)`, plus `saveParametersFileName`, `saveUrlVisualizer`, `saveUrlRemoteServer`. The overlay selectors call `switchLocation`/`switchMode` in `settingsActions.ts`; `probeAndSaveExecution` there probes, routes the payload to the per-location registry/params store, and records the status.
...
- **Electron storage keys** (`electron/utils/storage.ts`): `access_token`, `username`, `settings`, `colorMode` (default: `'light'`), `registered_nodes` (per-location `{ local, remote }`), `registered_network_nodes`, `execution_parameters` (per-location params), `jobs`, `jobIdMap`
