...
- `jobsStore.svelte.ts` - Job tracking. `jobsState` has `isEmpty`/`oneOrLess` getters, `update()` refreshes the list for the currently active location (IPC `list-local-runs` for `local`, SSH `sacct` for `remote`). `jobIdMapState` maps scheduler job IDs (Slurm ID remote, internal ID local — no external scheduler) → a `JobIdEntry` (`internalId` used in touch-dir paths, `backendKind`, and the per-job `workingDirectory`). The `workingDirectory` is the root for single runs and the per-stage subdirectory for pipeline stages, for both local and remote; `getOutFileContent` / `getNodesExecutionStatus` resolve a job's `slurm-<id>.out` and `nodes-exec-status/` from it (via `getJobWorkingDirectory` / `getEntryByInternal`), so logs and node status work uniformly for both.
...
- `src/lib/orchestration/pipelineOrchestrator.ts` - `runPipelineRemote(pipeline, runName)`: topo-orders the DAG and submits each stage as its own Slurm job into `<workingDirectory>/pipeline-<slug-or-ts>/stage-<id>`, chained via `--dependency=afterok` on its parents' Slurm ids, then polls all jobs concurrently for live feedback. Remote (Slurm) only for now.
- Per-stage submit primitives live in `sshMessages.ts`: `submitCoralStageRemote()` / `submitExecutableStageRemote()` build+upload+submit a single stage (with `sbatch --parsable --kill-on-invalid-dep=yes [--dependency=afterok:…]`) and return the Slurm id without polling. The existing single-run remote functions route through these. `withMpiPlugin(network, useMpi)` injects the MPI plugin block; `jobPolling()` is exported for reuse. `ensureUniqueRemoteDir(dir)` settles a run/pipeline's final output directory: it accepts an optional user-supplied name (slugified via `slugify.ts`'s `buildDirName(prefix, name?)`, falling back to a timestamp), and auto-appends a timestamp suffix rather than overwriting if that name's directory already exists.
...
- **Slurm batch scripts**: `buildSbatchScript()` in `src/lib/utils/sbatchScript.ts` builds every sbatch script — coral or executable, MPI or serial — from one shape: the `#SBATCH` header, plus a launcher prefix before the command. MPI adds `--nodes`/`--ntasks-per-node` (from `JobConfig`; defaults 1 node, 4 tasks/node, 01:00:00 time limit) and the launcher; serial jobs get neither. The two remote submit functions in `sshMessages.ts` call it directly with their own command string (the executable one shell-quotes its paths). The `.sh` templates and their Vite `?raw` imports are gone.
- **MPI launcher**: how ranks are started is a per-target setting (`mpiLauncher` on `ExecutionTargetSettings`: `kind: 'srun' | 'mpirun'` plus optional `extraArgs`, appended verbatim — that is also how a specific PMI plugin is forced, with `--mpi=pmix`). Defaults live in `createDefaultSettings()` (remote `srun`, local `mpirun`) and are backfilled into older stored settings by `normalizeStoredSettings()` in `settingsStore.svelte.ts`, the same way `probes` is — so the field is required and every reader can use it directly. `sshMessages.ts` reads it from settings at submit time rather than capturing it into `JobConfig`, since it belongs to the cluster, not the run. No rank count is ever passed: under Slurm both launchers take it from the allocation. The probe checks that the configured launcher is on the remote PATH and folds a warning into the probe message; it does not enumerate PMI plugins (`srun --mpi=list` is a one-line shell command for anyone who needs it).
- The single sidebar Run button (`SidebarButtons.svelte`) is context-aware: in single mode it opens `JobConfigModal.svelte` (renamed from `MpiConfigModal.svelte`) — which shows MPI-specific fields (nodes, tasks/node) only when MPI is enabled, always shows the time limit field, plus an optional run-name field (local and remote) used to name the run's output folder; in pipeline mode it opens `PipelineRunNameModal.svelte` and calls `runPipelineRemote`. The pipeline stage/import-export controls also live in `SidebarButtons` (shown in pipeline mode), so `PipelineCanvas.svelte` is a pure canvas. Execution location/mode are chosen from the interactive `ExecutionBadge.svelte` dropdowns in the top-right overlay (`App.svelte`), not from Settings.
