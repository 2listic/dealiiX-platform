 ...
 */
export async function simulateDragToCanvas(page: Page, sourceSelector: string): Promise<void> {
  await page.evaluate((src: string) => {
    const source = document.querySelector(src)
    const target = document.querySelector('.svelte-flow')
    if (!source || !target) throw new Error(`simulateDragToCanvas: element not found (${src})`)
    ...
    target.dispatchEvent(new DragEvent('dragover', { dataTransfer: dt, bubbles: true, clientX: x, clientY: y }))
    target.dispatchEvent(new DragEvent('drop', { dataTransfer: dt, bubbles: true, clientX: x, clientY: y }))
